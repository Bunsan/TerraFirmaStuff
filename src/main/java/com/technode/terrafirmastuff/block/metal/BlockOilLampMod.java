...
public class BlockOilLampMod extends BlockTerraContainer
{
    ...
    public IIcon getIcon(int side, int meta) {
        ...
        if (side == 0 || side == 1) {
            if (m == 0)
                return TFC_Textures.sheetBismuthBronze;
            else if (m == 1)
                return TFC_Textures.sheetBlackBronze;
            else if (m == 2)
                return TFC_Textures.sheetBlackSteel;
            else if (m == 3)
                return TFC_Textures.sheetBronze;
            else if (m == 4)
                return TFC_Textures.sheetBrass;
            else if (m == 5)
                return TFC_Textures.sheetRedSteel;

            else return TFC_Textures.sheetBismuthBronze;
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        icons = new IIcon[6];
        icons[0] = registerer.registerIcon(ModDetails.ModID + ":" + "metal/BismuthBronzeLamp");
        icons[1] = registerer.registerIcon(ModDetails.ModID + ":" + "metal/BlackBronzeLamp");
        icons[2] = registerer.registerIcon(ModDetails.ModID + ":" + "metal/BlackSteelLamp");
        icons[3] = registerer.registerIcon(ModDetails.ModID + ":" + "metal/BronzeLamp");
        icons[4] = registerer.registerIcon(ModDetails.ModID + ":" + "metal/BrassLamp");
        icons[5] = registerer.registerIcon(ModDetails.ModID + ":" + "metal/RedSteelLamp");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
    {
        if(!world.isRemote)
        {
            ...
            if(!isLampLit(meta))
            {
                TEOilLampMod te = (TEOilLampMod) world.getTileEntity(x, y, z);
                if (te != null)
                {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack is)
    {
        ...
        if (te instanceof TEOilLampMod)
        {
            ((TEOilLampMod)te).create();
            FluidStack fs = FluidStack.loadFluidStackFromNBT(is.getTagCompound());
            if(fs != null)
            {
                ((TEOilLampMod)te).setFuelFromStack(fs);
            }
			/*else
			{
				//world.setBlockMetadataWithNotify(x, y, z, world.getBlockMetadata(x, y, z)+8, 0x3);
			}*/
            ((TEOilLampMod)te).hourPlaced = (int) TFC_Time.getTotalHours();
