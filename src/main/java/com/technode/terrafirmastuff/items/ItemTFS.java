...
public class ItemTFS extends Item implements ISize
{
    ...
    public ItemTFS setMetaNames(String[] metanames)
    {
    ...
    public ItemTFS setCraftingXP(int m)
    {
        craftingXP = m;
        return this;
    }
    ...
    public ItemTFS setFolder(String s)
    {
    ...
    public static void addSizeInformation(ItemStack object, List<String> arraylist)
    {
        if(((ISize)object.getItem()).getSize(object)!= null && ((ISize)object.getItem()).getWeight(object) != null && ((ISize)object.getItem()).getReach(object)!= null)
            arraylist.add("\u2696" + Reference.translation("gui.Weight." + ((ISize)object.getItem()).getWeight(object).getName()) + " \u21F2" +
                    Reference.translation("gui.Size." + ((ISize)object.getItem()).getSize(object).getName().replace(" ", "")));
        if(object.getItem() instanceof IEquipable)
        {
            if(((IEquipable)object.getItem()).getEquipType(object) == IEquipable.EquipType.BACK)
            {
                arraylist.add(EnumChatFormatting.LIGHT_PURPLE.toString() + Reference.translation("gui.slot")+
                        EnumChatFormatting.GRAY.toString()+": " +
                        EnumChatFormatting.WHITE.toString() + Reference.translation("gui.slot.back"));
            }
        }
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag)
    {
        ...
        ItemTFS.addSizeInformation(is, arraylist);

        ItemTFS.addHeatInformation(is, arraylist);

        if (is.hasTagCompound())
        {
            if(is.getTagCompound().hasKey("itemCraftingValue") && is.getTagCompound().getShort("itemCraftingValue") != 0)
                arraylist.add(Reference.translation("gui.ItemWorked"));
        }
...
//    }
    ...
    public ItemTFS setSize(EnumSize e)
    {
        size = e;
        return this;
    }

    public ItemTFS setWeight(EnumWeight e)
    {
        weight = e;
        return this;
    }
